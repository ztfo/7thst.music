...
console.log(`✓ Connected to Cloudinary: ${cloudName}\n`);
...
async function fetchImagesByTag(tag) {
  try {
    ...
    console.log(`  ✓ Found ${taggedImages.length} images with tag: ${tag}`);
    ...
    console.error(`  ❌ Error fetching images for tag "${tag}":`, error.message || error);
    if (error.http_code) {
...
`;
  ...
  console.log(`\n📝 Next step: Update art.html to include images-data.js`);
}
